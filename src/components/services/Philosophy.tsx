      ...
      <div className="container-main" ref={ref}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {items.map((item, i) => (
            <div
              key={i}
              className="bg-white p-8 flex flex-col gap-4 group hover:shadow-lg transition-all duration-500 opacity-0 animate-fade-up"
              style={{ border: "1px solid rgba(0,0,0,0.06)", animationDelay: `${0.2 + i * 0.1}s` }}
            >
              <span className="text-gold/40 font-light" style={{ fontSize: "42px", lineHeight: 1 }}>{item.number}</span>
